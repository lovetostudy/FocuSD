...
- **待办事项**：永久保留，不按日期拆分。完成时自动归档到本地 Markdown 文件。支持设专注任务。
- **完成日记书**：按日期查看已完成事项，卡片/时间线两种布局，数据直接读磁盘文件。
- **Markdown 保存**：未完成列表自动保存为 `todos.md`，已完成按日期追加到 `YYYY-MM-DD.md`。默认保存目录为安装目录下 `todos/`，可在设置中自定义。
...
从 GitHub Releases 下载 `FocuSD Island_0.1.0_x64-setup.exe`，按提示安装。
