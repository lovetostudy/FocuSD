...
> Windows 桌面悬浮岛（Tauri + React），把最重要任务、AI Agent 状态灯和媒体控制放在屏幕顶部。支持多设备待办同步。

## 核心功能

- **悬浮岛窗口**：透明、无边框、始终置顶。自适应内容宽度（180~400px），支持左右偏移。折叠态为单行胶囊，展开后进入各功能页面。
- **AI Agent 状态灯**：胶囊内嵌红/绿指示灯（支持多个并发 session）。安装 hooks 后，多个 Claude Code 实例各自独立亮灯。
...
- **多设备同步**：通过 HTTP 服务器同步待办数据（pull-then-push 协议）。编辑后自动推送、启动时自动拉取。
- **媒体控制**：系统音频活动检测，播放/暂停、上一首、下一首。
- **外观设置**：透明度、缩放、上下边距、左右偏移、主题颜色，支持自定义预设。
- **系统集成**：托盘菜单、开机自启动。
...
│   ├── src/lib.rs          # 原生命令、窗口定位、托盘、Agent hooks、媒体、文件 I/O、同步
